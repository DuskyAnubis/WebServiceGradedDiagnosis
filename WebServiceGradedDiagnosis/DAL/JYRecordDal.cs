                    ...
                    if (dtJy != null && dtJy.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dtJy.Rows.Count; i++)
                        {
                            JYRecord jYRecord = new JYRecord
                            {
                                ...
                                IsKey = "0",
                                CheckType = "1",
                                CheckTime = Convert.ToDateTime(dtJy.Rows[i]["zxsj"]).ToString("yyyy-MM-dd hh:mm:ss"),
                                ReportTime = Convert.ToDateTime(dtJy.Rows[i]["zxsj"]).ToString("yyyy-MM-dd hh:mm:ss"),
                                ReceiveTime = Convert.ToDateTime(dtJy.Rows[i]["sqsj"]).ToString("yyyy-MM-dd hh:mm:ss"),
                                CollectTime = Convert.ToDateTime(dtJy.Rows[i]["sqsj"]).ToString("yyyy-MM-dd hh:mm:ss"),
                                Diagnostic = dtPatBaseInf != null && dtPatBaseInf.Rows.Count > 0 && dtPatBaseInf.Rows[0]["Diagnosis1"].ToString() != "" ? dtPatBaseInf.Rows[0]["Diagnosis1"].ToString() : "暂无",
                                State = "0",
                    ...
                    if (dtJy != null && dtJy.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dtJy.Rows.Count; i++)
                        {
                            JYRecord jYRecord = new JYRecord
                            {
                                ...
                                Diagnostic = dtMzbl != null && dtMzbl.Rows.Count > 0 && dtMzbl.Rows[0]["西医诊断"].ToString() != "" ? dtMzbl.Rows[0]["西医诊断"].ToString() : "暂无",
                                State = "0",
                                PatientName = dtGh.Rows[0]["姓名"].ToString(),
                                GenderValue = dtGh.Rows[0]["性别"].ToString() == "男" ? "1" : "0",
                                PatientAge = dtGh.Rows[0]["年龄"].ToString(),
                                AppDiagonse = dtMzbl != null && dtMzbl.Rows.Count > 0 && dtMzbl.Rows[0]["西医诊断"].ToString() != "" ? dtMzbl.Rows[0]["西医诊断"].ToString() : "暂无",
                                ChiefComplaint = dtMzbl != null && dtMzbl.Rows.Count > 0 && dtMzbl.Rows[0]["主诉"].ToString() != "" ? dtMzbl.Rows[0]["主诉"].ToString() : "暂无",
                                MedicalHistory = "暂无",
                                CheckPurpose = dtJy.Rows[i]["sqxmmc"].ToString(),
                                Other1 = null,
                                Other2 = null,
